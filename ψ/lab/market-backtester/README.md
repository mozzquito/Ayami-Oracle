...
- **Correlation guard** (added 2026-08-15, agy's suggestion after a zcode/agy roadmap review): originally a per-market cap (`{"crypto": 2, "forex": 2}`), **redesigned 2026-09-08** into a shared total across every market — `config.MAX_CONCURRENT_POSITIONS = 5` — plus `config.MAX_PER_MARKET = {"crypto": 4}` so crypto can't claim the entire shared pool. Crypto symbols move together often enough that a rally can fire many entry signals in one run — that's one leveraged bet on "crypto goes up" wearing N costumes, not N independent ones. `advise(..., allow_entry=bool)` gates entries; `cloud_run.py` counts currently-open positions (both a running total and a per-market count) before the loop and keeps both live as entries/exits happen *within* the same run, so a signal past either cap gets logged as blocked (visible in `railway logs`) but doesn't open a position or trigger a Discord ping.
  - **Why a shared pool**: the real constraint is account capital ($10/position), not which market a position happens to be in — the user reported $50 in their real exchange account on 2026-09-08, i.e. 5 positions' worth, system-wide. A per-market split doesn't reflect that; a shared total does.
  - **Why crypto still has its own sub-cap**: a pure shared pool of 5 has a failure mode zcode and agy both independently flagged — crypto has 9 symbols (vs forex's 3) and tends to signal together, so an unguarded shared pool lets crypto claim every slot the instant a rally fires 5+ signals at once, leaving forex zero room regardless of its own signal quality (pure first-come-first-served bias from `SYMBOLS` list order, crypto listed first). `MAX_PER_MARKET["crypto"] = 4` guarantees forex at least 1 of the shared 5 even in that worst case.
  - **Known limitation (not yet fixed)**: within a single market's own allotment, whichever symbol comes first in `cloud_run.py`'s `SYMBOLS` list order still claims the slot — not the strongest signal. agy flagged this (2026-09-08) when reviewing the first cap increase; a future improvement would collect all of a run's bullish signals first, rank by signal strength (e.g. RSRS score, RSI level), then fill available slots with the strongest ones instead of first-come-first-served.
  - **Cap history**: 2 (both markets, from launch) → crypto raised to 3 earlier on 2026-09-08 (watchlist grew 6→9 crypto symbols on 2026-09-01 without the cap keeping pace, 3 real signals observed blocked simultaneously) → redesigned same day into the shared-pool-of-5-with-crypto-sub-cap-4 model above once real account capital ($50) was reported. Max system-wide exposure is now $50 (paper capital, $10/position) instead of the old per-market split's worst case of $30 crypto + $20 forex = $50 (same total, different allocation — now capital-driven instead of arbitrary per-market splits).
