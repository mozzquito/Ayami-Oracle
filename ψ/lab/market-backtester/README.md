...
Runs `python -m backtester.cloud_run` — loops **8 crypto paper-trade positions** (no real money): BTC, ETH, SOL, TRX, BNB, NEAR, POL, SHIB, 24/7 in the cloud (doesn't depend on any local machine being on). Symbols were picked by actually running the strategy's live signal / historical return / % time bullish against real candidates and keeping the strongest — not guessed (see `ψ/memory/retrospectives/` or session history). Symbol list + params live in `backtester/cloud_run.py`'s `SYMBOLS`.

**Forex removed 2026-09-08** (EURUSD, GBPUSD, AUDUSD) — user's explicit call, trading crypto exclusively in real life now. Two of the three had real open paper positions at removal time (GBPUSD since 2026-08-27, AUDUSD since 2026-09-02); removed immediately per the user's choice rather than waiting for them to close naturally, so their `.state/` files on the Railway volume are now orphaned — frozen at last-checked state forever, harmless since it's paper capital only. The CLI itself still fully supports `--market forex` for standalone `run`/`advise`/`min-capital` use (see Usage above); only the always-on cron watchlist is now crypto-only.

**DOGE and AVAX removed, POL added (2026-09-16)** — after Boss asked why the live watchlist kept losing money and authorized the assembled zcode/agy research team to investigate and act autonomously overnight. Two independent checks converged on the same conclusion for DOGE/AVAX: (1) they were the worst live paper performers of the whole watchlist (DOGE -14.1%, AVAX -4.8% equity since inception) and (2) a walk-forward out-of-sample test (train on data ≤2026-06-30, test on 2026-07-01→09-16, exact production params, no re-optimization) showed both went **negative with a 0% win rate** out-of-sample — confirming their original +55.6%/+78.4% headline numbers were survivorship-biased maxima from the 2026-09-01 10-candidate comparison, not a real edge. AVAX had a real open paper position at removal time (entered 2026-09-15); removed immediately per the same precedent set for forex, so its `.state/` file is now orphaned too.

POL (Polygon, formerly MATIC) was the **only** one of the originally-rejected 2026-09-01 candidates (also re-checked: LTC, LINK, MATIC's own `rsrs_trend` variant) that showed genuinely convergent evidence — positive on both the full 2023-2026 backtest (+18.4%, Sharpe 0.34) **and** the same held-out out-of-sample window that sank DOGE/AVAX (+20.52%, Sharpe 2.83). Every other candidate looked good in the short out-of-sample window alone but was negative over the full multi-year history — the same short-window-looks-good trap already diagnosed above, so none of them were added despite tempting recent numbers. Only one replacement was added (not two, to fill both freed slots) — no other candidate cleared this bar, and forcing a second one in just to keep the symbol count at 9 would have repeated the exact mistake being corrected.

**Ticker quirk**: yfinance still keys Polygon's token under the pre-rebrand CoinMarketCap-style symbol `POL28321` (`POL-USD` and `MATIC-USD` are both dead/delisted on yfinance) — `cloud_run.py`'s `SYMBOLS` entry, the `.state/` filename, and Discord report headers all say "POL28321" as a result. This is intentional, not a typo. The Discord quick-trade link is unaffected — `advisor.py`'s `_BINANCE_SYMBOL_OVERRIDE` maps it back to the real `POL_USDT` pair (verified against Binance's own `exchangeInfo` API: `POLUSDT` is `TRADING`, `MATICUSDT` is `BREAK`/halted).
...
- **Correlation guard** (added 2026-08-15, agy's suggestion after a zcode/agy roadmap review): `config.MAX_CONCURRENT_POSITIONS = 5` — a shared total across every market in `SYMBOLS` (just crypto now, see below). Crypto symbols move together often enough that a rally can fire many entry signals in one run — that's one leveraged bet on "crypto goes up" wearing N costumes, not N independent ones. `advise(..., allow_entry=bool)` gates entries; `cloud_run.py` counts currently-open positions before the loop and keeps the count live as entries/exits happen *within* the same run, so a signal past the cap gets logged as blocked (visible in `railway logs`) but doesn't open a position or trigger a Discord ping.
  - **Why a shared total, not a fixed per-symbol/market split**: the real constraint is account capital ($10/position), not which symbol or market a position happens to be in — the user reported $50 in their real exchange account on 2026-09-08, i.e. 5 positions' worth. `MAX_CONCURRENT_POSITIONS` reflects that capital directly.
  - **`config.MAX_PER_MARKET` (currently empty `{}`)**: when forex was still in the watchlist, this held `{"crypto": 4}` — a sub-cap protecting forex's share of the shared pool from a crypto rally claiming every slot (a real failure mode zcode and agy both independently flagged: crypto had 9 symbols vs forex's 3, tended to signal together, and processed first in `SYMBOLS` list order, so an unprotected shared pool let crypto starve forex out entirely). With forex removed 2026-09-08 (crypto-only watchlist now), there's no second market left to protect, so this dict is empty and crypto uses the full 5. The guard logic still reads it generically (`MAX_PER_MARKET.get(market)`), so a future market addition needs no code change, just a value here.
  - **List-order allocation — investigated 2026-09-16, kept as-is**: within the watchlist, whichever symbol comes first in `cloud_run.py`'s `SYMBOLS` list order still claims a slot first, not the strongest signal — agy flagged this as a theoretical gap on 2026-09-08. Backtested properly before touching production code (this touches the same entry-decision control flow that had the forming-bar bug, so it got the same research-first treatment as the symbol-selection question below): across the full available history for all 8 watchlist symbols, list order only actually diverges from strength-ranked allocation on ~35 days total (~4-5 times/year) — the shared cap of 5 is rarely the binding constraint in the first place. Worse, strength-ranking (by RSRS magnitude / RSI distance / momentum) **underperformed** plain list order by -4.5% to -10.2% aggregate return across every ranking method tried, because it systematically over-allocated to high-beta alts (SHIB, NEAR) during broad rallies, which then got stopped out more often — the current `SYMBOLS` order (majors first) turned out to accidentally function as a quality/liquidity hierarchy. Not implementing signal-strength ranking; the "limitation" costs effectively nothing and a naive fix would have made things slightly worse.
  - **Cap history**: 2 per market (from launch) → crypto raised to 3 (watchlist grew 6→9 crypto symbols on 2026-09-01 without the cap keeping pace, 3 real signals observed blocked simultaneously) → redesigned into a shared pool of 5 with a crypto sub-cap of 4 once real account capital ($50) was reported → forex removed entirely the same day (2026-09-08, user trading crypto exclusively now), simplifying back to crypto alone using the full shared 5.

**Research tried and rejected (2026-09-16)** — recorded so this isn't re-investigated from scratch later. Boss asked the assembled zcode/agy research team to look for ways to improve profitability after the forming-bar bug fix and the DOGE/AVAX→POL watchlist change (both above). Two more ideas were properly backtested and **not adopted**:
- **BTC-200-day-SMA regime gate**, tested two ways: blocking new entries while BTC < its 200-SMA, and force-closing all open positions on the same condition. Both made things *worse*, not better — aggregate return dropped 32.5pp (entry-only) to 44.7pp (force-flat), and max drawdown got *worse* too (the one metric the gate exists to improve), because it specifically blocked/force-exited the recovery-phase entries `rsrs_trend` (TRX, SHIB) profits from. Full multi-year history, pre-registered 200-day rule, not tuned.
- **Fractional/Kelly position sizing** vs. the current fixed 100%-of-cash sizing (existing `config.py` defaults, not re-tuned): both reduce max drawdown substantially (aggregate -24.5% → -9.5%/-1.8%) but *also* reduce risk-adjusted return (Sharpe 0.82 → 0.67/0.69) — they just shrink exposure, not improve trade selection. Kelly in particular is working exactly as designed: these strategies' sub-50% win rates correctly estimate a thin edge, so Kelly sizes near its 2% floor almost permanently. Kept `fixed` in production.
...
**Known bug (fixed 2026-09-16): live signals were being decided on a still-forming daily bar.** The strategies are daily-bar strategies — the backtest engine only ever sees a bar after it's fully closed — but `advise()` fetches OHLCV with an open-ended end date and read `signal.iloc[-1]` directly, so an hourly cron call (every symbol, every hour, `0 * * * *`) was very often evaluating "today's" still-forming candle as if it were a completed close. Discovered 2026-09-16 after Boss reported the live paper equity had been net negative across nearly the whole watchlist since launch (7 of 9 symbols, -6.0% aggregate). zcode (consulted for a second opinion) identified the mechanism from reading `advisor.py`; the diagnosis was then independently confirmed by replaying the exact live window (2026-08-11→2026-09-16) through this same backtest engine using only completed daily bars — **every single symbol's faithful replay beat its actual live result**, by margins up to 24 percentage points (average: backtest replay +7.3% vs. live actual -6.0% over the identical calendar window and real market data). That gap — not bad luck, not "no edge," since the replay used the same real prices — is what the fix below closes.

**The fix**: `advise()` now drops the last OHLCV row before computing anything if that row's date is today (UTC) — signal generation, `as_of`, and the fill price for new entries are always based on the last **fully-closed** daily bar, exactly matching what the backtest engine would have seen. Stop-loss/take-profit monitoring for an already-open position was moved to the **live quote** (`yfinance`'s `fast_info`, already fetched for the staleness check) instead of the daily bar's High/Low, specifically so this fix doesn't cost a day of responsiveness on an actual stop/target hit — falls back to the completed bar's High/Low only if the live fetch fails. Verified via: a fresh-state dry run showing `as_of` correctly one day behind "today" on every symbol; a forced-stop unit test confirming the live-price path fires correctly; and the same test with the live fetch mocked out to confirm the fallback path still works.

**Not yet re-measured**: whether this fix closes most of the -6.0% gap or the strategies still need work — needs a few more weeks of live data (or a fresh replay once enough post-fix history accumulates) before that's answerable. See `ψ/memory/retrospectives/` for the full zcode/agy consultation and the replay methodology.
