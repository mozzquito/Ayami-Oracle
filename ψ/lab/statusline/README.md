...
(offline, 150 checks, runs the script under macOS `/bin/bash` 3.2). Real Claude Code stdin captured as a redacted fixture: `fixtures/live-2.1.278.json`.
...
🎯 statusline dev (04:22) • 🧊 cache cold (~145k to re-cache) • ⏳2 (1 stale)          <- only when there is something to say
```
...
| 🧊 cold cache | stdin `prompt_cache.warm == false` | cache went cold: next message re-writes the whole context at full price |
...
**Rollback:** delete the `"statusLine"` block from `.claude/settings.json` (the global script is untouched and takes over again).
